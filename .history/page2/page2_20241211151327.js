...
function animate() {
	...
	// Add random slight rotations to other visualizers
	for (let i = 5; i <= 15; i++) {
		const vis = meshes[`visualizer${i}`]
		vis.outer.rotation.x += (Math.random() - 0.5) * 0.01
		vis.outer.rotation.y += (Math.random() - 0.5) * 0.01
		vis.outer.rotation.z += (Math.random() - 0.5) * 0.01
	}
	...
	btn.on('click', () => {
		...
		gsap.to(audio, {
			...
			onComplete: () => {
				...
				// Only run color animation if we have a selected visualizer
				if (selected && meshes[`visualizer${selected}`]) {
					...
					// Add each color transition to the timeline
					colors.forEach((color, index) => {
						...
						// Also animate emissive color with slight delay
						colorTimeline.to(
							material.emissive,
							{
								r: color.r * 0.5,
