	...
	if (intersects.length > 0) {
		if (!audioFlag) {
			...
			initializeAudio().then(() => {
				// Now you can start playing songs
				handleSongSwap(intersects[0].object.userData.url)
			})
			// const analyzer = analyzeAudio();
			// await analyzer.initAudio();
			// await analyzer.swapSongs('song.mp3');
			// await analyser.initAudio()
			// await analyser.swapSongs(intersects[0].object.userData.url)
			prev = active
...
function handleSongSwap(songUrl) {
	return analyzer.swapSongs(songUrl).catch((error) => {
