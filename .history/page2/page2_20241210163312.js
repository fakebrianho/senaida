...
function onClick(event) {
	...
	if (intersects.length > 0) {
		...
		// if (selectFlag) {
		// meshes[
		// 	`visualizer${clickedNum}`
		// ].outer.material.uniforms.uColor1.value = new THREE.Color(0xfcba03)
		// }
...
function animate() {
	...
	meshes.visualizer1.outer.rotation.x += 0.005
	meshes.visualizer2.outer.rotation.y += 0.005
	meshes.visualizer3.outer.rotation.y -= 0.005
	meshes.visualizer4.outer.rotation.z -= 0.005
	...
	if (selectFlag) {
		if (PARAMS.LoFi) {
			meshes[
				`visualizer${selected}`
			].outer.material.uniforms.uColor1.value = new THREE.Color(0xf02616)
		} else {
			meshes[
				`visualizer${selected}`
			].outer.material.uniforms.uColor1.value = new THREE.Color(0xfcba03)
