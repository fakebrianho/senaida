...
// Add this new class to manage audio effects
class AudioEffects {
	constructor() {
		// Lo-fi effects chain
		...
		// Initialize normal chain (bypass effects)
	...
	async loadSong(url) {
		try {
			...
			const audioUrl = new URL(url, window.location.href).href
			...
			console.log('Song loaded:', audioUrl)
		} catch (error) {
	...
	disableLoFi() {
		// Disconnect lo-fi chain
		this.player.disconnect()

		// Reconnect normal chain
		this.player.connect(Tone.Destination)
	}
	...
	try {
		...
		if (intersects.length > 0) {
			if (!audioFlag) {
				...
				// Initialize audio effects with error handling
				audioEffects = new AudioEffects()
				try {
					await audioEffects.loadSong(
						intersects[0].object.userData.url
					)

					if (PARAMS.lofiMode) {
						audioEffects.enableLoFi()
					}

					await audioEffects.play()
					...
					stopAudio() // Clean up if loading fails
				}
			} else if (active !== intersects[0].object.userData.num) {
				try {
					await audioEffects.loadSong(
						intersects[0].object.userData.url
					)
...
function listeners() {
	...
	window.addEventListener('keydown', (event) => {
		...
		// Add Lo-fi mode toggle with 'L' key
		if (event.key.toLowerCase() === 'l') {
			PARAMS.lofiMode = !PARAMS.lofiMode
			if (audioEffects) {
				if (PARAMS.lofiMode) {
					audioEffects.enableLoFi()
				} else {
					audioEffects.disableLoFi()
				}
			}
		}
	})
		...
		try {
			...
			if (data) {
				...
				vis.inner.material.uniforms.uAmp.value =
					data.frequency * PARAMS.amplitude
...
function setupPane() {
	...
	// Add Lo-fi controls
	const lofiFolder = pane.addFolder({ title: 'Lo-fi Effects' })

	lofiFolder.addBinding(PARAMS, 'lofiMode', {
		label: 'Lo-fi Mode',
		onChange: (value) => {
			if (audioEffects) {
				if (value) {
	...
	lofiFolder.addBinding(PARAMS, 'bitDepth', {
		min: 1,
		max: 16,
		step: 1,
		label: 'Bit Depth',
		onChange: (value) => {
			if (audioEffects) {
				audioEffects.bitCrusher.bits = value
			}
		},
	})

	lofiFolder.addBinding(PARAMS, 'sampleRate', {
		min: 0.1,
		max: 1,
		label: 'Sample Rate',
		onChange: (value) => {
			if (audioEffects) {
				audioEffects.bitCrusher.wet.value = value
			}
		},
	})
