	...
	if (audioFlag && analyser && active && meshes[`visualizer${active}`]) {
		try {
			...
			if (data) {
				...
				vis.inner.material.uniform.uFreq.value = PARAMS.frequency
				vis.inner.material.uniform.uAmp.value = PARAMS.amplitude
				vis.outer.material.uniform.uFreq.value = PARAMS.frequency
				vis.outer.material.uniform.uAmp.value = PARAMS.amplitude
