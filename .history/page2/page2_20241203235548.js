...
function onClick(event) {
	...
	if (intersects.length > 0) {
		console.log(meshes[`visualizer${intersects[0].object.userData.num}`])
		// meshes[`visualizer${active}`]
		gsap.to(
			meshes[`visualizer${intersects[0].object.userData.num}`].inner
				.position,
			{
				x: 0,
				y: 0,
		...
		gsap.to(
			meshes[`visualizer${intersects[0].object.userData.num}`].outer
				.position,
			{
				x: 0,
				y: 0,
