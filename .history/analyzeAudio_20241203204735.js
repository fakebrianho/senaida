...
export function analyzeAudio(url) {
	...
	async function initAudio() {
		try {
			...
			await reverb.generate() // Wait for reverb to generate its impulse response
			...
			// Create Tone.js player with initial buffer
			tonePlayer = new Tone.Player({
				url: url ? `/audio/${url}` : undefined,
				autostart: false,
			}).connect(filter)
	...
	async function swapSongs(_url) {
		try {
			if (!tonePlayer) {
				console.warn('TonePlayer not initialized')
				return
			}
			const audioUrl = `/audio/${_url}`
			await tonePlayer.load(audioUrl)
			tonePlayer.start()
		} catch (error) {
