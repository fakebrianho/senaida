...
export default class Model {
	...
	init() {
		this.loader.load(this.file, (gltf) => {
			...
			const color = this.textureLoader.load('/Obsidian_001_basecolor.png')
	...
	obsidianMaterial() {
		...
		const height = this.textureLoader.load(
			'/obsidian/Obsidian_001_height.png)'
		)
		const roughness = this.textureLoader.load(
			'/obsidian/Obsidian_001_roughness.jpg'
		)
		const normal = this.textureLoader.load(
			'/obsidian/Obsidian_001_normal.jpg'
		)
		const material = new MeshPhysicalMaterial({
			map: color,
			metalness: 0.5,
