...
function animate() {
	...
	if (loadedFlag && interactables.length > 0) {
		...
		if (intersects.length > 0) {
			currentMouse.x += (interactables[0].uv.x - currentMouse.x) * 0.05
			currentMouse.y += (interactables[0].uv.y - currentMouse.y) * 0.05
			meshes.portal.material.uniforms.uMouse.value.set(
				currentMouse.x,
				currentMouse.y
			)
		} else {
			currentMouse.x += (defaultVector.x - currentMouse.x) * 0.05
			currentMouse.y += (defaultVector.y - currentMouse.y) * 0.05
			meshes.portal.material.uniforms.uMouse.value.set(
				currentMouse.x,
				currentMouse.y
			)
