...
export function createParticles(count = 750, size = 0.015, camera) {
	...
	for (let i = 0; i < count * 3; i += 3) {
		...
		if (Math.abs(positions[i + 2]) < 2.0) {
			positions[i + 2] += positions[i + 2] < 0 ? -2.0 : 2.0
		}
	...
	const particlesMaterial = new ShaderMaterial({
		...
		// alphaTest: 0.5, // Discards fragments with alpha < 0.5
		// depthTest: true,
		// depthWrite: true,
	})
